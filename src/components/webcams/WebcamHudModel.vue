 ...
 * (3.8 MB `cube_accel4000.gcode`: 0.03 ms); only a file that has none is read to the end, and
 * the largest one on this machine, 3.0 MB of pure motion, takes under 10 ms. Both measured.
 *
 ...
 *
 * No whitespace is required in front of the E. Every slicer spaces its words and every file
 * on this machine does too (checked), but `G1X10E5` is legal g-code and klipper takes it -
 * and the diagnostics here are written by hand. Requiring the space would answer "no
 * extrusion" for a file that has plenty, which is a worse lie than the one being fixed. It
 * cannot over-match either: the search is anchored to a G0/G1 and stops at the comment, so
 * everything it walks is that move's own words, where E means the extruder and nothing else.
 */
const extrudingMove = /(?:^|\n)[ \t]*[gG](?:0|1|00|01)(?![0-9.])[^;\n]*?[eE](-?[0-9]*\.?[0-9]+)/g
